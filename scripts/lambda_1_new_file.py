...
def file_validation(file_name, file_type, file_size, upload_date):
    ...
    for info in file_info:
        if info['file_name'] == file_name:
            ...
            if info['file_size'] == file_size and info['file_type'] == file_type:
                return file_exists
            else:
                file_exists = False
...
def lambda_handler(event, context):
    ...
    file_exists = file_validation(file_name, file_type, file_size, upload_date)
    ...
    if file_exists:
        ...
        if "meta" in file_name:
            print('Crawler meta (ppa-crawler-s3-raw-meta) starting...')
            response = client.start_crawler(Name='ppa-crawler-s3-raw-meta')
            print(json.dumps(response))
        else:
            print('Crawler review (ppa-crawler-s3-raw-review) starting...')
            response = client.start_crawler(Name='ppa-crawler-s3-raw-review')
            print(json.dumps(response))